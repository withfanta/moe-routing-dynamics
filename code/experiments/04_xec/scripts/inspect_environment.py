"""XEC-P0 environment and provenance audit. Computes NO research metric."""
...
from xec.cache_features import build_projection, projection_metadata
...
def main():
    ...
    gpus = gpu_table()
    idle = [g for g in gpus
            if "V100" in g.get("name", "")
    ...
    print("\nGPUs detected:")
    for g in gpus:
        print(f"  {g.get('index')}: {g.get('name')} free {g.get('memory_free')} "
              f"util {g.get('utilization')} uuid {g.get('uuid')}")
    print("idle V100 candidates (>25 GiB free):", env["idle_v100_candidates"])
    ...
    prov = {
        ...
        "projection": projection_metadata(P),
    }
    ...
    print("projection sha256:", prov["projection"]["sha256"])

    if mismatches or minfo.sha != MODEL_REVISION:
        print("\nSTOP: provenance mismatch", mismatches)
        return 1
    if len(idle) < 2:
        print(f"\nWARNING: only {len(idle)} idle V100 candidates; two are needed")
